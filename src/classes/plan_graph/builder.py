...
class PlanBuilder:
    ...
    def add_step(self, tool_name: str, args_json: str, result: str) -> None:
        """
        Add a step (tool call) under the current branch.
        If no branch is set, creates an implicit "General" branch under root.
        """
        with self._lock:
            if self._root is None:
                return
            ...
            if branch_id is None:
                general = _make_node("branch", "General", parent_id=self._root["id"])
    ...
    def get_current_branch(self) -> Optional[str]:
        """Return current branch id (under lock)."""
        with self._lock:
            return self._current_branch_id

    def get_plan_json(self) -> Optional[Dict[str, Any]]:
        """Return the current plan as a JSON-serializable dict, or None."""
        with self._lock:
            if self._root is None:
                return None
    ...
    def get_breakdown(self) -> Dict[str, List[Dict]]:
        """Return a breakdown by category (imaging, script, manim, music)."""
        with self._lock:
            if self._root is None:
                return {}
        plan = self.get_plan_json()
