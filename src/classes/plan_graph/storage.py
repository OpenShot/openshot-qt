...
def _db_path():
    """Get the database path for plan history."""
...
def save_plan(plan_builder):
    """Persist the current plan to SQLite. Returns the row ID or None."""
    try:
...
def load_plan(plan_id: int) -> Optional[Dict[str, Any]]:
    """Load a plan by its database ID."""
    try:
        path = _db_path()
