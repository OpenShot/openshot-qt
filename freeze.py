...
if sys.platform == "win32":
   ...
   # Add libresvg (if found)
    ...
    # Add QtWebEngineProcess (if found)
    from PyQt5.QtCore import QLibraryInfo
    _qt_libs = QLibraryInfo.location(QLibraryInfo.LibrariesPath)
    qt_install_path = os.path.dirname(_qt_libs)  # parent of lib/
    qt_webengine_path = os.path.join(_qt_libs, "QtWebEngineCore.framework", "Versions", "5")
    ...
    # Add QtWebEngineProcess Resources & Local
    _webengine_resources = os.path.join(qt_webengine_path, "Resources")
    if os.path.exists(_webengine_resources):
        for filename in find_files(_webengine_resources, ["*"]):
            external_so_files.append((filename, os.path.relpath(filename, start=_webengine_resources)))
        for filename in find_files(os.path.join(_webengine_resources, "qtwebengine_locales"), ["*"]):
            external_so_files.append((filename, os.path.relpath(filename, start=_webengine_resources)))
    _qt_plugins = os.path.join(qt_install_path, "plugins")
    for filename in find_files(_qt_plugins, ["*"]) if os.path.exists(_qt_plugins) else []:
